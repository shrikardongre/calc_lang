...
void Lexer :: next (Token &token) {
   ...
   if (!*BufferPtr) {              // <- this checks for '\0'
    token.Kind = Token::eoi;
    token.Text = llvm::StringRef(BufferPtr, 0); // empty text is fine
    return;
}
    ...
    if (charinfo :: isLetter(*BufferPtr)){
        ...
        if(name=="with") token.Kind=Token::with ;
        else token.Kind =Token::ident ;
        formToken(token,end,token.Kind);
        return;
    }

...
class Parser {
    ...
    void error(){
        llvm::errs()<< "  Unexpected Token :" << "kind "<<Tok.getkind()<<" text "<<Tok.getText() <<"\n" ;
        Error=true;
    }

    void advance(){Lex.next((Tok));}
    // In the expect method we are checking if someting goes wrong , and if something is wrong then we return true.
    ...
    // If the Token kind is the expected one then we will simply advance().
